...
#         v2=actor.tenant_id,  # we dont need to remmeber user_03 is admin
...
def get_all_policies_of_resource(
    db: Session, resource_id: str, tenant_id: str
) -> List[CasbinRule]:
    query = db.query(CasbinRule).filter(
        and_(
            CasbinRule.ptype == PolicyTypeEnum.p,
...
# well there is no grouping in the new design
# def get_grouping(db: Session, role_id: str, user_id: str) -> CasbinRule:
#     query = db.query(CasbinRule).filter(
#         and_(
#             CasbinRule.ptype == PolicyTypeEnum.g,
#             CasbinRule.v0 == user_id,
#             CasbinRule.v1 == role_id,
#         )
#     )
#     db_item = query.first()
#     if db_item is None:
...
def delete_policies_with_resource_id(db: Session, resource_id: str) -> None:
    ...
    db_items = db.query(CasbinRule).filter(CasbinRule.v1 == resource_id)
